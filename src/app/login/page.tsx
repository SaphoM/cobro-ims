...
import { getSession, getMfaStepUpState } from '@/lib/auth';
...
export default async function LoginPage({
  ...
  // An authenticated session that has already cleared MFA (or has none to
  // clear) goes straight through. One that is still pending step-up stays here
  // and is shown the 6-digit code form directly — it must not slip past.
  let stepUpPending = false;
  if (session) {
    ({ stepUpPending } = await getMfaStepUpState());
    if (!stepUpPending) redirect(safeNext ?? '/dashboard');
  }
