...
/**
 * PORTED from src/app/login/page.tsx + login-form.tsx. Same card, same
 * gradient backdrop, same role-picker autofill, same show/hide password eye,
 * same oversized (deliberately NOT 36px) email/password fields.
 *
 * The one honest difference from the original is called out on the page
 * itself: sign-in here is a client-side demo check against the same four
 * demo accounts, not real authentication.
 */
export function LoginPage() {
  ...
  // Prefill the primary demo account so the page is immediately usable,
  // matching what the original's autofill buttons put there on first click.
  useEffect(() => {
    if (emailRef.current && !emailRef.current.value) emailRef.current.value = DEMO_ACCOUNTS[0].email;
    if (passwordRef.current && !passwordRef.current.value) passwordRef.current.value = DEMO_ACCOUNTS[0].password;
  }, []);
  ...
  return (
    <div className="flex min-h-screen flex-1 flex-col items-center justify-center gap-7 bg-[radial-gradient(58%_46%_at_50%_34%,rgba(240,161,60,0.17),transparent_70%),radial-gradient(120%_85%_at_50%_105%,rgba(240,161,60,0.06),transparent_60%)] px-5 py-12">
      <main className="relative w-full max-w-[404px] rounded-[22px] border border-accent/[0.14] bg-gradient-to-b from-surface to-bg-2 px-9 pt-11 pb-8 shadow-[0_40px_80px_-32px_rgba(0,0,0,0.7)]">
        ...
        {error && (
          <div role="alert" className="mb-4 rounded-[10px] border border-danger/40 bg-danger/10 px-3 py-2.5 text-[0.84rem] text-[#f3a99a]">
            {error}
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-[17px]">
          ...
          <label className="flex flex-col gap-2">
            <span className="flex items-baseline justify-between gap-3">
              ...
              <span className="text-[0.8rem] text-text-faint">Forgot password?</span>
            </span>
        ...
        <p className="mt-6 rounded-lg border border-accent/[0.14] bg-surface-2 px-3 py-2.5 text-center text-[0.76rem] leading-relaxed text-text-faint">
          <strong className="text-text-muted">Demo sign-in.</strong> This static build checks the
          credentials above in your browser and keeps the session on this device. It is not real
          authentication and protects nothing — the live system uses server-side Supabase Auth.
        </p>
      </main>
      ...
      <p className="text-center font-mono-brand text-[0.68rem] font-medium tracking-[0.08em] text-text-faint uppercase">
        ...
        <span>v{packageJson.version} · static demo</span>
      </p>
    </div>
  );
}
