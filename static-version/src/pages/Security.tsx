...
/**
 * PORTED AS-IS from src/app/dashboard/security/page.tsx.
 *
 * The 2FA indicator in the original was already correct and is reproduced
 * unchanged: a coloured status badge ("2FA enabled" in accent / "2FA not
 * enabled" in danger red) plus a state-specific panel, both driven from the
 * live `mfaEnrolled` flag on the user record — never a separate UI boolean.
 * There are exactly TWO states in the data model; no "pending/setup" state
 * is invented here because the system doesn't have one.
 *
 * The gate this drives is real: `approve_adjustments` is refused with the
 * identical message when mfaEnrolled is false (see permissions.ts).
 * Enrollment itself is mock in the original too — it sets the same flag a
 * real Supabase Auth MFA flow would set, with no authenticator app involved.
 */
export function SecurityPage() {
  const session = useCurrentUser();
  const setMfaEnrolled = useStore((s) => s.setMfaEnrolled);
    ...
    <div className="flex flex-col gap-6">
      <div>
        ...
        <p className="text-[0.86rem] text-text-muted">
          Two-factor authentication for privileged users, per the RFQ. This is a mock enrollment flow — no
          real authenticator app is involved, only the flag a real Supabase Auth MFA flow would set — but it
          gates the same action a real 2FA requirement would.
        </p>
      </div>
